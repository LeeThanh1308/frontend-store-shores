...
function Branches() {
  ...
  const { branches, branch, validators, onRefresh } =
  ...
  const onSubmit = useCallback(
    (data) => {
      if (showFormCreated) dispatch(handleCreateBranch(data));
      if (showFormUpdated) {
        dispatch(handleUpadateBranch({ ...data, id: idThisUpdated }));
      }
    },
  ...
  useEffect(() => {
    const timerID = setTimeout(() => {
      if (branches?.length == 0) dispatch(handleGetBranches());
    }, 200);
            ...
            <span
              ...
              onClick={() => {
                if (window.confirm("Bạn chắc chắn muốn xóa chi nhánh này?"))
                  dispatch(handleDeleteBranch({ id: record?.id }));
              }}
            >
              Delete
            </span>
      ...
      <div className="p-6 z-0">
        <div className="flex justify-between items-center">
          <div className="mb-4 font-bold text-5xl text-rose-700">
            ...
            <span className=" text-3xl">({branches?.length} chi nhánh)</span>
          </div>
          <div
            ...
            <Button
              ...
              onClick={() => {
                ...
                if (window.confirm("Bạn chắc chắn muốn xóa chi nhánh đã chọn?"))
                  dispatch(handleDeleteBranch({ ids: idsSelectedRows }));
              }}
              danger
            >
              Xóa nhiều
